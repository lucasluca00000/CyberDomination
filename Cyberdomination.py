...
class Player(pygame.sprite.Sprite):
    ...
    def load_sprites(self):
        ...
        try:
            for i in range(1, 3):
                img = pygame.image.load(f'personnages/idle_{i}.png').convert_alpha()
                sprites['idle'].append(pygame.transform.scale(img, (self.width, self.height)))
            for i in range(1, 5):
                img = pygame.image.load(f'personnages/walk_{i}.png').convert_alpha()
                sprites['walk'].append(pygame.transform.scale(img, (self.width, self.height)))
            for i in range(1, 5):
                img = pygame.image.load(f'personnages/run_{i}.png').convert_alpha()
                sprites['run'].append(pygame.transform.scale(img, (self.width, self.height)))
            for i in range(1, 5):
                img = pygame.image.load(f'personnages/attack_{i}.png').convert_alpha()
                sprites['attack'].append(pygame.transform.scale(img, (self.width, self.height)))
            for i in range(1, 4):
                img = pygame.image.load(f'personnages/hurt_{i}.png').convert_alpha()
...
class Enemy(pygame.sprite.Sprite):
    ...
    def shoot(self, player, projectiles):
        if self.shoot_cooldown <= 0 and self.can_see_player(player):
            projectile = Projectile(
                self.rect.centerx, self.rect.centery,
                player.rect.centerx, player.rect.centery, speed=6
            )
            projectiles.add(projectile)
            self.shoot_cooldown = self.shoot_delay
            self.shoot_delay = random.randint(60, 150)
...
class Game:
    ...
    def load_backgrounds(self):
        ...
        try:
            ...
            print("Arrière-plans chargés avec succès")
        except pygame.error as e:
            print(f"Erreur: {e} - Utilisation de l'arrière-plan par défaut")
            return None
        return backgrounds
    ...
    def draw_background(self):
        if self.backgrounds:
            for i, bg in enumerate(self.backgrounds):
                offset_x = -(self.player.rect.x // (3 + i)) % SCREEN_WIDTH if self.player else 0
                screen.blit(bg, (offset_x, 0))
                if offset_x < 0:
                    screen.blit(bg, (offset_x + SCREEN_WIDTH, 0))
        else:
            for i in range(SCREEN_HEIGHT):
                r = int(30 + (i / SCREEN_HEIGHT) * 40)
                g = int(20 + (i / SCREEN_HEIGHT) * 30)
                b = int(60 + (i / SCREEN_HEIGHT) * 80)
                pygame.draw.line(screen, (r, g, b), (0, i), (SCREEN_WIDTH, i))
    
    def draw_hud(self):
        pygame.draw.rect(screen, (20, 10, 40, 200), (0, 0, SCREEN_WIDTH, 50))
    ...
    def run(self):
        ...
        while running:
            ...
            if self.state == GameState.MENU:
                ...
                restart_text = restart_font.render("Appuyez sur ENTREE pour recommencer", True, WHITE)
